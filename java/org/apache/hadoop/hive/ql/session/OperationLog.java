...
public class OperationLog {
  ...
  private class LogFile {
    ...
    synchronized void write(String msg) {
      ...
      /**
       *
       * @param isFetchFirst true表示从文件头开始抓去数据
       * @param maxRows  抓去多少行数据
       * @return
       * @throws SQLException
       */
   ...
   //将输入流关闭掉
    ...
    private List<String> readResults(long nLines) throws SQLException {
      if (in == null) {
        try {
          in = new BufferedReader(new InputStreamReader(new FileInputStream(file)));//重新打开文件流
