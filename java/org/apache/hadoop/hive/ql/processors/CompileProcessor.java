 ...
 * 使用GROOVY,对jvm进行编译,生成一个jar包,并且将jar包添加到session中
 */
public class CompileProcessor implements CommandProcessor {
  ...
  public static final String IO_TMP_DIR = "java.io.tmpdir";//存储临时目录
  public static final String GROOVY = "GROOVY";
  public static final String AS = "AS";
  public static final String NAMED = "NAMED";
  private static final String SYNTAX = "syntax: COMPILE ` some code here ` AS groovy NAMED something.groovy";//有一些语法错误
  private static final AtomicInteger runCount;//运行次数
  ...
  /**
   * 编译 `代码` AS groovy NAMED xxx.groovy
   ...
   * ADD RESOURCE command.编译并且打包到一个jar内,然后将jar添加到session里面
  ...
  public CommandProcessorResponse run(String command) throws CommandNeedRetryException {
    ...
    if(authErrResp != null){//说明有异常,则返回即可
    ...
    try {
      parse(ss);
    } catch (CompileProcessorException e) {//出现异常了,则返回即可
  ...
  void parse(SessionState ss) throws CompileProcessorException {
    ...
    int startPosition = 0;//`字符的开始位置
    int endPosition = -1;//`字符的结束位置
    ...
    for (int i = startPosition; i < command.length(); i++) {
      if (command.charAt(i) == '\\') {
        ...
        i = i + 1;
        continue;
      } else if (command.charAt(i) == '`'){//找到`结尾字符
        endPosition = i;
          break;
      } else {
    ...
    if (endPosition == -1){
      throw new CompileProcessorException(SYNTAX);//没有找到`字符,因此抛异常
    }
    StringTokenizer st = new StringTokenizer(command.substring(endPosition+1), " ");//按照空格拆分后续的命令
    if (st.countTokens() != 4){
      throw new CompileProcessorException(SYNTAX);
    }
    String shouldBeAs = st.nextToken();
    if (!shouldBeAs.equalsIgnoreCase(AS)){ //必须是as关键字
      throw new CompileProcessorException(SYNTAX);
    }
    setLang(st.nextToken());
    if (!lang.equalsIgnoreCase(GROOVY)){ //必须是GROOVY关键字
      throw new CompileProcessorException("Can not compile " + lang + ". Hive can only compile " + GROOVY);
    }
    String shouldBeNamed = st.nextToken();
    if (!shouldBeNamed.equalsIgnoreCase(NAMED)){//必须是NAMED关键字
      throw new CompileProcessorException(SYNTAX);
    }
