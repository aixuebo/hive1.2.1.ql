...
/**
 * SetProcessor.
 * 获取某一个属性信息 设置key=value信息 获取全部属性信息
 *
 demo:
 set ;
 set -v;
...
public class SetProcessor implements CommandProcessor {
  ...
  private void dumpOptions(Properties p) {
    ...
    // Inserting hive variables 插入hivevar:的环境信息
  ...
  public static int setVariable(String varname, String varvalue) throws Exception {
    ...
    if (varname.startsWith(ENV_PREFIX)){//不能设置系统环境变量
      ss.err.println("env:* variables can not be set.");
      return 1;
    } else if (varname.startsWith(SYSTEM_PREFIX)){//可以设置系统属性信息
  ...
  public CommandProcessorResponse run(String command) {
    ...
    if (nwcmd.equals("")) {//如果命令是空,则返回所有的配置信息
      dumpOptions(ss.getConf().getChangedProperties());
      return createProcessorSuccessResponse();
    }

    if (nwcmd.equals("-v")) {//如果命令是-v,则打印conf信息
      dumpOptions(ss.getConf().getAllProperties());
      return createProcessorSuccessResponse();
    }

    String[] part = new String[2];//存储key和value
    int eqIndex = nwcmd.indexOf('=');

    if (nwcmd.contains("=")){ //包含=,则表示是要设置key=value信息
    ...
    try {
      return getVariable(nwcmd);//没有等号,表示要获取key对应的值
    } catch (Exception e) {
...
// create a Schema object containing the give column
  private Schema getSchema() {
    ...
    tmpFieldSchema.setName(SET_COLUMN_NAME);//set属性

    tmpFieldSchema.setType(STRING_TYPE_NAME);//String类型
    sch.putToProperties(SERIALIZATION_NULL_FORMAT, defaultNullString);//设置属性,对null值设置什么内容
