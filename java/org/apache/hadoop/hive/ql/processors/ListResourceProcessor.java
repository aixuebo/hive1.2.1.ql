...
public class ListResourceProcessor implements CommandProcessor {
  ...
  static {
    SCHEMA = new Schema();
    SCHEMA.addToFieldSchemas(new FieldSchema(LIST_COLUMN_NAME, STRING_TYPE_NAME, null));//设置resource属性,String类型
    SCHEMA.putToProperties(SERIALIZATION_NULL_FORMAT, defaultNullString);//设置serialization.null.format = \N,即默认空字符串是什么
  }
  ...
  public CommandProcessorResponse run(String command) throws CommandNeedRetryException {
    ...
    if (tokens.length > 1) {//获取除了第0个之外的剩余数组信息
      filter = Arrays.asList(Arrays.copyOfRange(tokens, 1, tokens.length));
    }
    Set<String> s = ss.list_resource(t, filter);//查询信息
    if (s != null && !s.isEmpty()) {//打印信息结果
